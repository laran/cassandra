...
Materialized view names are defined by:
...
The select statement of a materialized view creation defines which columns of the base tables are included in the view.
The statement is limited in a number of ways:

- the :ref:`SELECT <selection-clause>` is limited to columns of the base table. In other
  words, you can't use any function (aggregate or not), casting, term, etc. Aliases are also not supported. You can
  however use `*` as a shortcut to select all columns. Further, :ref:`static columns <static-columns>` cannot be
...
- it can have neither an :ref:`ORDER BY <ordering-clause>`, nor a :ref:`LIMIT <limit-clause>`, nor an :ref:`ALLOW
...
- it must contain all the primary key columns of the base table. This ensures that every row of the view corresponds to
...
A materialized view is internally implemented by a table and as such, creating a MV allows the :ref:`same options as
...
The options that can be updated are the same as at creation time and thus the :ref:`same as for tables
...
Dropping a materialized view uses the ``DROP MATERIALIZED VIEW`` statement:
